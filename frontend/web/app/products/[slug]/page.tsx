...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="shell storefront"><Link className="health-link" href="/products">← Back to catalog</Link><article className="product-detail"><p className="eyebrow">{product.brand?.name ?? "Catalog product"}</p><h1>{product.name}</h1><p className="intro">{product.description ?? product.shortDescription ?? "Product description will be added by the catalog team."}</p><div className="detail-grid"><section><h2>Variants</h2>{product.variants.map((variant) => { const stock = availability.get(variant.id); return <div className="variant-row" key={variant.id}><span>{variant.name}<small>{stock ? `${stock.availability === "AVAILABLE" ? "Available" : "Out of stock"} (${stock.availableQuantity})` : "Availability unavailable"}</small></span><strong>{variant.price.amount} {variant.price.currency}</strong></div>; })}</section><section><h2>Categories</h2><p className="form-copy">{product.categories.map((category) => category.name).join(" · ") || "Uncategorized"}</p></section></div></article></main>;
}
